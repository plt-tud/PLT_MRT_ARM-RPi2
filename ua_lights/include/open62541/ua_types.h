...
#include <stdlib.h>
static UA_INLINE UA_String UA_STRING(const char *chars) {
    UA_String str; str.length = strlen(chars);
    str.data = (UA_Byte *) malloc(str.length); 
    strcpy( (char *) str.data, chars); return str; }
...
static UA_INLINE UA_QualifiedName UA_QUALIFIEDNAME(UA_UInt16 nsIndex, const char *chars) {
...
static UA_INLINE UA_LocalizedText UA_LOCALIZEDTEXT(const char *locale, const char *text) {
